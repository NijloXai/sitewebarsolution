/*
  Composant d'animation 3D "GridScan" - Effet de grille futuriste avec scan lumineux.
  
  Ce composant affiche une grille 3D animée avec un effet de scan qui parcourt la surface.
  L'animation réagit aux mouvements de souris de l'utilisateur pour créer un effet de parallaxe.
  
  L'utilisateur voit :
  - Une grille en perspective avec des lignes colorées
  - Un rayon lumineux qui balaye périodiquement la surface
  - Un effet de profondeur qui suit le curseur de la souris
  
  Options disponibles :
  - Suivi de la webcam pour réagir au visage (désactivé par défaut)
  - Gyroscope sur mobile
  - Effets visuels personnalisables (bloom, aberration chromatique)
*/
...
/* Shader vertex - Positionne les pixels à l'écran */
const vert = `
varying vec2 vUv;
void main(){
  vUv = uv;
...
/* Shader fragment - Dessine la grille 3D et l'effet de scan */
const frag = `
...
/* Composant principal d'animation GridScan */
export const GridScan = ({
  ...
  /* Ajoute un nouveau scan à la liste des scans actifs */
  ...
  /* Gestion des mouvements de souris pour l'effet de parallaxe */
  useEffect(() => {
    ...
    const onClick = async () => {
      ...
      if (
        ...
        // @ts-expect-error - DeviceOrientationEvent.requestPermission existe sur iOS
  ...
  /* Initialisation du rendu WebGL Three.js et boucle d'animation */
  useEffect(() => {
  ...
  /* Mise à jour des paramètres visuels en temps réel */
  useEffect(() => {
  ...
  /* Gestion du gyroscope sur mobile */
  useEffect(() => {
  ...
  /* Chargement des modèles de détection de visage (optionnel) */
  useEffect(() => {
    let canceled = false;
    const load = async () => {
      try {
  ...
  /* Détection de visage via webcam pour le suivi du regard */
  useEffect(() => {
    let stop = false;
    let lastDetect = 0;
        ...
        if (ts - lastDetect >= 33) {
          ...
          try {
            ...
            if (res && res.detection) {
              ...
              lookTarget.current.copy(look.multiplyScalar(depthScaleVal));
  ...
  return (
    <div ref={containerRef} className={`relative w-full h-full overflow-hidden ${className ?? ''}`} style={style}>
      {showPreview && (
        <div className="absolute right-3 bottom-3 w-[220px] h-[132px] rounded-lg overflow-hidden border border-white/25 shadow-preview bg-black text-white text-[12px] leading-[1.2] font-sans pointer-events-none">
...
/* Convertit une couleur hexadécimale en couleur linéaire pour le shader */
function srgbColor(hex: string) {
  const c = new THREE.Color(hex);
  return c.convertSRGBToLinear();
}

/* Amortissement fluide pour les vecteurs 2D (mouvement de caméra lisse) */
function smoothDampVec2(
  ...
  const targetCopy = current.clone().sub(change);
  const temp = currentVelocity.clone().addScaledVector(change, omega).multiplyScalar(deltaTime);
  currentVelocity.sub(temp.clone().multiplyScalar(omega));
  currentVelocity.multiplyScalar(exp);

  out.copy(targetCopy.clone().add(change.add(temp).multiplyScalar(exp)));
...
/* Amortissement fluide pour les valeurs numériques */
...
/* Ajoute une valeur au buffer et maintient sa taille maximale */
